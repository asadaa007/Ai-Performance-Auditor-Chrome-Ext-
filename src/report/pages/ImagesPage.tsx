...
export function ImagesPage() {
  return (
    <ReportPageShell title="Images" description="Image inventory and optimization flags">
      {(result) => <ImagesContent result={result} />}
    </ReportPageShell>
  );
}

function ImagesContent({ result }: { result: AuditResult }) {
  return (
    <div className="space-y-6">
      <Section title="Summary">
        <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
          ...
          />
          <MetricCard label="Modern format" value={formatNumber(result.images.modernFormatCount)} />
        </div>
      </Section>
